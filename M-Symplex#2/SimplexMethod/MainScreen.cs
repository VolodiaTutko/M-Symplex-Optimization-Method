...
namespace SimplexMethod
{
    public partial class MainScreen : System.Windows.Forms.Form
    {
        ...
        private void Dual_Click(object sender, EventArgs e)
        {
            DualWindow dualWindow = new DualWindow();

            
            double[][] coefficients = new double[constraintsCount][];
            double[] constants = new double[constraintsCount];
            string[] signs = new string[constraintsCount];
            for (int i = 0; i < constraintsCount; i++)
            {
                coefficients[i] = new double[variablesCount];
                for (int j = 0; j < variablesCount; j++)
                {
                    coefficients[i][j] = Convert.ToDouble(constraintsGridView.Rows[i].Cells[j].Value);
                }
                constants[i] = Convert.ToDouble(constraintsGridView.Rows[i].Cells[variablesCount + 1].Value);
                signs[i] = Convert.ToString(constraintsGridView.Rows[i].Cells[variablesCount].Value);
            }

          
            double[] functionCoefficients = new double[variablesCount];
            for (int i = 0; i < variablesCount; i++)
            {
                functionCoefficients[i] = Convert.ToDouble(functionGridView.Rows[0].Cells[i].Value);
            }

            
            bool isMax = extrComboBox.SelectedIndex == 0; 

            
            dualWindow.ShowConstraints(coefficients, functionCoefficients, signs, constants, isMax);

           
            dualWindow.Show();
        }

        private void functionGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
